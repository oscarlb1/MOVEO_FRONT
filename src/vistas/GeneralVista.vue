...
const chartOptionsArea = computed<ApexOptions>(() => ({
  ...
  xaxis: {
    ...
    labels: { style: { colors: temaStore.isDark ? '#82A1B1' : '#757575' } }
  },
  yaxis: { labels: { style: { colors: temaStore.isDark ? '#82A1B1' : '#757575' } } },
  grid: { borderColor: temaStore.isDark ? '#374B54' : '#EEEEEE', strokeDashArray: 4 },
  tooltip: { theme: temaStore.isDark ? 'dark' : 'light', style: { fontSize: '12px' }, marker: { show: true } },
  legend: { show: false },
}))
  ...
  plotOptions: {
    pie: {
      donut: {
        ...
        labels: {
          show: true,
          name: { color: temaStore.isDark ? '#82A1B1' : '#757575' }, // Arreglo para que Mantenimiento sea legible
          value: { color: temaStore.isDark ? '#ffffff' : '#424242', fontWeight: '700', fontSize: '20px' },
          total: { show: true, label: 'Total', color: temaStore.isDark ? '#82A1B1' : '#757575' },
        }
      }
    }
  },
...
function colorEstadoEntrega(estado: string) {
  if (estado === 'ENTREGADO') return { bg: 'bg-green-50 dark:bg-green-900/20', text: 'text-green-700 dark:text-green-400', border: 'border-green-200 dark:border-green-800' }
  if (estado === 'EN_CAMINO') return { bg: 'bg-blue-50 dark:bg-blue-900/20', text: 'text-blue-700 dark:text-blue-400', border: 'border-blue-200 dark:border-blue-800' }
  if (estado === 'FALLIDO') return { bg: 'bg-red-50 dark:bg-red-900/20', text: 'text-red-700 dark:text-red-400', border: 'border-red-200 dark:border-red-800' }
...
function colorNotificacion(titulo: string) {
  ...
  if (t.includes('error') || t.includes('fallo') || t.includes('retraso')) return { border: 'border-l-red-500', icon: 'text-red-500', bg: 'bg-red-50 dark:bg-red-900/20' }
  if (t.includes('alerta') || t.includes('mantenimiento') || t.includes('revisión')) return { border: 'border-l-yellow-500', icon: 'text-yellow-500', bg: 'bg-yellow-50 dark:bg-yellow-900/20' }
  if (t.includes('alcanzada') || t.includes('exitosa') || t.includes('completada')) return { border: 'border-l-green-500', icon: 'text-green-500', bg: 'bg-green-50 dark:bg-green-900/20' }
  return { border: 'border-l-blue-500', icon: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-900/20' }
}
...
function colorPosicion(pos: number) {
  if (pos === 1) return 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'
  if (pos === 2) return 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'
  if (pos === 3) return 'bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400'
      ...
      if (ubicacion) {
        ...
        if (marcadoresVehiculos.has(ruta.id)) {
          ...
          const icon = L.divIcon({
            ...
            html: `<div class="w-4 h-4 rounded-full shadow-lg border-2 ${temaStore.isDark ? 'border-[#272A30]' : 'border-white'}" style="background-color: #E67E50;">
          ...
          marker.bindTooltip(tooltipContent, {
            direction: 'top',
            offset: [0, -14],
            className: temaStore.isDark ? 'custom-dark-tooltip' : 'custom-light-tooltip',
            opacity: 1
          })
...
<template>
  <div class="space-y-6 pb-10">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 animacion-entrada">
      ...
      <div v-if="!cargando && usuariosActivos.length > 0" class="flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-medium bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-700 text-green-700 dark:text-green-400">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
      ...
      <template v-else>
        <div v-for="(kpi, i) in kpis" :key="i" class="p-5 rounded-2xl border transition-all shadow-sm bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#E67E50]">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
      <div class="lg:col-span-3 p-6 rounded-2xl border shadow-sm bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#374B54] animacion-entrada overflow-hidden min-w-0" style="animation-delay: 100ms">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h2 class="font-bold text-lg text-[#092C4C] dark:text-white">Entregas por Día</h2>
            <p class="text-xs mt-0.5 text-[#9e9e9e] dark:text-[#82A1B1]">Distribución semanal de entregas totales vs completadas</p>
          </div>
          <div class="flex gap-4 text-xs font-semibold">
            <span class="flex items-center gap-1.5"><span class="w-3 h-1.5 rounded-full bg-[#E67E50]"></span> <span class="text-[#757575] dark:text-[#82A1B1]">Entregas</span></span>
            <span class="flex items-center gap-1.5"><span class="w-3 h-1.5 rounded-full bg-[#374B54]"></span> <span class="text-[#757575] dark:text-[#82A1B1]">Completadas</span></span>
          </div>
        </div>
        <div v-if="cargando" class="h-[300px] rounded-xl animate-pulse bg-gray-50 dark:bg-[#16181A]"></div>
        <div v-else class="w-full h-[300px]">
          <VueApexCharts height="100%" width="100%" :options="chartOptionsArea" :series="chartSeriesArea" />
        </div>
      </div>

      <div class="lg:col-span-2 p-6 rounded-2xl border shadow-sm bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#374B54] animacion-entrada overflow-hidden min-w-0" style="animation-delay: 200ms">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 p-6 rounded-2xl border shadow-sm bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#374B54] animacion-entrada min-w-0" style="animation-delay: 300ms">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h2 class="font-bold text-lg mb-1 text-[#092C4C] dark:text-white">Mapa en Tiempo Real</h2>
            <div class="flex items-center gap-2 text-sm">
              <div class="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
              <span class="text-[#757575] dark:text-[#82A1B1]">
      ...
      <div class="p-6 rounded-2xl border shadow-sm bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#E67E50] animacion-entrada min-w-0" style="animation-delay: 400ms">
        ...
        <div class="mt-8 p-4 rounded-xl border border-transparent bg-gray-50 dark:bg-[#E67E50]/5 dark:border-[#E67E50]/20">
          ...
          <p class="text-xs mt-1 text-[#757575] dark:text-[#82A1B1]">Basado en entregas recientes</p>
        </div>
      </div>
    </div>

    <div class="grid lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 rounded-2xl border shadow-sm bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#374B54] animacion-entrada min-w-0 overflow-hidden" style="animation-delay: 450ms">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between p-6 pb-4 gap-4">
          ...
          <div class="flex items-center gap-2">
            <div class="relative">
              ...
              <input v-model="busquedaEntregas" type="text" placeholder="Buscar..." class="pl-9 pr-4 py-1.5 text-sm border rounded-lg focus:outline-none focus:border-[#E67E50] dark:focus:border-[#E67E50] transition-colors bg-transparent border-gray-200 dark:border-[#374B54] text-[#424242] dark:text-white placeholder-gray-500" />
            </div>
          </div>
        </div>
          ...
          <table class="w-full text-sm min-w-[700px]">
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-[#374B54]">
              <tr v-if="entregasFiltradas.length === 0">
                <td colspan="6" class="py-10 text-center text-sm text-gray-400 dark:text-gray-500">No hay entregas registradas</td>
              </tr>
              <tr v-for="e in entregasFiltradas" :key="e.id" class="transition-colors hover:bg-gray-50 dark:hover:bg-[#16181A]/50">
                <td class="py-4 px-6 font-semibold text-xs text-[#424242] dark:text-gray-300">#D-{{ e.id }}</td>
                <td class="py-4 px-4"><p class="font-medium text-[#424242] dark:text-white">{{ e.cliente?.nombre || `Cliente #${e.clienteId}` }}</p></td>
                <td class="py-4 px-4 text-[#757575] dark:text-[#82A1B1]">Ruta #{{ e.rutaId }}</td>
                ...
                <td class="py-4 px-4 text-xs text-[#757575] dark:text-[#82A1B1]"><span class="flex items-center gap-1"><Clock class="w-3.5 h-3.5" />{{ horaEntrega(e.horaEntregaReal || e.updatedAt) }}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="rounded-2xl border shadow-sm bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#374B54] animacion-entrada min-w-0 overflow-hidden" style="animation-delay: 500ms">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between p-6 pb-4 gap-4">
          ...
          <div class="flex items-center gap-2">
            <template v-if="conteoNoLeidas > 0">
              ...
              <button @click="marcarTodasLeidas" class="text-xs font-medium transition-colors text-gray-400 hover:text-[#424242] dark:hover:text-white">Marcar todo</button>
            </template>
            <button @click="mostrarBroadcast = !mostrarBroadcast" class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all" :class="mostrarBroadcast ? 'bg-[#E67E50] text-white' : 'bg-gray-100 dark:bg-[#16181A] text-gray-600 dark:text-gray-300 hover:bg-[#E67E50] hover:text-white'"><Send class="w-3.5 h-3.5" />Nueva</button>
          </div>
        </div>
        <div v-if="mostrarBroadcast" class="mx-6 mb-4 p-4 rounded-xl border border-orange-100 dark:border-[#E67E50]/30 bg-orange-50 dark:bg-[#E67E50]/10">
          <p class="text-xs font-bold mb-3 flex items-center gap-1.5 text-[#E67E50] dark:text-orange-400">
            <Send class="w-3.5 h-3.5" />Notificación Global (todos los usuarios)
          </p>
          <input v-model="broadcastTitulo" type="text" placeholder="Título..." maxlength="100" class="w-full px-3 py-2 text-sm border rounded-lg mb-2 focus:outline-none focus:border-[#E67E50] dark:focus:border-[#E67E50] bg-transparent transition-colors border-orange-200 dark:border-[#374B54] text-[#424242] dark:text-white placeholder-gray-500" />
          <textarea v-model="broadcastMensaje" rows="2" placeholder="Mensaje para todos los repartidores..." maxlength="500" class="w-full px-3 py-2 text-sm border rounded-lg mb-3 focus:outline-none focus:border-[#E67E50] dark:focus:border-[#E67E50] bg-transparent resize-none transition-colors border-orange-200 dark:border-[#374B54] text-[#424242] dark:text-white placeholder-gray-500" />
          ...
          <div class="flex gap-2">
            ...
            <button @click="mostrarBroadcast = false" class="px-4 py-2 rounded-lg text-xs font-semibold transition-colors bg-gray-200 dark:bg-[#16181A] text-gray-600 dark:text-[#82A1B1] hover:bg-gray-300 dark:hover:bg-[#374B54]">Cancelar</button>
          </div>
        </div>
        <div v-if="cargando" class="px-6 pb-6 space-y-3">
        ...
        <div v-else class="px-6 pb-6 space-y-3 max-h-[450px] overflow-y-auto custom-scrollbar">
          ...
          <div v-for="n in notificacionesRecientes" :key="n.id" class="p-4 rounded-xl border-l-4 transition-all hover:shadow-sm bg-white dark:bg-[#16181A]/50 border-r border-t border-b border-gray-100 dark:border-r-[#374B54] dark:border-t-[#374B54] dark:border-b-[#374B54]" :class="[colorNotificacion(n.titulo).border, n.leido ? 'opacity-60' : '']">
            <div class="flex items-start gap-3">
              ...
              <button v-if="!n.leido" @click="marcarLeida(n.id)" class="p-1 rounded-full opacity-50 hover:opacity-100 transition-opacity flex-shrink-0" title="Marcar como leída"><X class="w-3.5 h-3.5 text-gray-400 dark:text-[#82A1B1]" /></button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="grid lg:grid-cols-2 gap-6">
      <div class="rounded-2xl border shadow-sm p-6 bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#374B54] animacion-entrada min-w-0 overflow-hidden" style="animation-delay: 550ms">
        ...
        <div v-else>
          <div v-if="vehiculos.length === 0" class="text-center py-8 text-sm text-gray-400 dark:text-gray-500">Sin datos de flota</div>
          <div v-else>
            ...
            <div class="space-y-3 mb-6">
              <div v-for="item in estadoFlota" :key="item.label" class="flex items-center justify-between py-2 border-b last:border-0 border-gray-50 dark:border-[#374B54]">
                <div class="flex items-center gap-3">
                  <div class="w-3 h-3 rounded-full flex-shrink-0" :style="{ backgroundColor: item.color }"></div>
                  <span class="text-sm text-[#757575] dark:text-[#82A1B1]">{{ item.label }}</span>
                </div>
                <div class="flex items-center gap-3">
                  <div class="w-24 h-1.5 rounded-full overflow-hidden bg-gray-100 dark:bg-[#16181A]">
                  ...
                  <span class="text-sm font-bold w-6 text-right text-[#424242] dark:text-white">{{ item.val }}</span>
                </div>
              </div>
            </div>
            <div class="pt-4 border-t border-gray-100 dark:border-[#374B54]">
              <h3 class="text-sm font-semibold mb-3 flex items-center gap-2 text-[#424242] dark:text-white">
                <Wrench class="w-4 h-4 text-amber-500" />Mantenimientos Recientes
              </h3>
              <div v-if="mantenimientosRecientes.length === 0" class="text-xs py-3 text-center text-gray-400 dark:text-gray-500">Sin mantenimientos registrados</div>
              <div v-else class="space-y-2">
                <div v-for="m in mantenimientosRecientes" :key="m.id" class="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-[#16181A]/50">
                  <div class="min-w-0">
                    <p class="text-xs font-semibold truncate text-[#424242] dark:text-white">{{ m.tipoMantenimiento }}</p>
                    <p class="text-[10px] mt-0.5 text-gray-500 dark:text-[#82A1B1]">Vehículo #{{ m.vehiculoId }} · {{ fechaMantenimiento(m.fechaServicio) }}</p>
                  </div>
                  <span class="text-xs font-bold ml-3 flex-shrink-0 text-amber-600 dark:text-amber-400">{{ m.coste.toFixed(0) }}€</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="rounded-2xl border shadow-sm p-6 bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#374B54] animacion-entrada min-w-0 overflow-hidden" style="animation-delay: 600ms">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-5 gap-4">
          ...
          <div class="flex items-center gap-1 p-1 rounded-lg bg-gray-100 dark:bg-[#16181A]">
            ...
            <button @click="cambiarSortRanking('puntos')" class="px-3 py-1.5 rounded-md text-xs font-semibold transition-all" :class="rankingSortBy === 'puntos' ? 'bg-[#E67E50] text-white shadow-sm' : 'text-gray-500 dark:text-[#82A1B1] hover:text-[#424242] dark:hover:text-white'">Puntos</button>
          </div>
        </div>
        <div v-if="cargando || cargandoRanking" class="space-y-4">
          <div v-for="i in 5" :key="i" class="h-16 rounded-xl animate-pulse bg-gray-100 dark:bg-[#16181A]"></div>
        </div>
        <div v-else>
          <div v-if="ranking.length === 0" class="text-center py-8 text-sm text-gray-400 dark:text-gray-500">Sin datos de ranking</div>
          <div v-else class="space-y-3">
            <div v-for="r in ranking" :key="r.usuarioId" class="flex items-center gap-4 p-4 rounded-xl border transition-all hover:shadow-sm border-gray-100 dark:border-[#374B54] hover:bg-gray-50 dark:hover:bg-[#16181A]/50">
              ...
              <div class="flex-1 min-w-0">
                <p class="font-semibold text-sm truncate text-[#424242] dark:text-white">{{ r.nombreUsuario }}</p>
                <p class="text-xs text-[#9e9e9e] dark:text-[#82A1B1]">
              ...
              <div class="text-right flex-shrink-0">
                ...
                <p class="text-[10px] text-gray-400 dark:text-[#82A1B1]">{{ rankingSortBy === 'puntos' ? 'puntos' : 'entregas' }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="rounded-2xl border shadow-sm p-6 bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#374B54] animacion-entrada overflow-hidden min-w-0" style="animation-delay: 650ms">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-5 gap-4">
        ...
        <span class="text-sm font-medium px-3 py-1 rounded-full bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400">
      ...
      <div v-if="cargando" class="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        <div v-for="i in 4" :key="i" class="h-20 rounded-xl animate-pulse bg-gray-100 dark:bg-[#16181A]"></div>
      </div>
      <div v-else>
        <p v-if="repartidoresActivos.length === 0" class="text-sm py-6 text-center text-gray-400 dark:text-gray-500">No hay datos de sesiones activas</p>
        <div v-else class="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        <div v-for="u in repartidoresActivos" :key="u.usuarioId" class="flex items-center gap-3 p-4 rounded-xl border transition-all bg-gray-50 dark:bg-[#16181A]/50 border-gray-100 dark:border-gray-700" :class="u.estaActivo ? '' : 'opacity-50'">
            <div class="relative flex-shrink-0">
              ...
              <span class="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 bg-gray-400 dark:bg-[#16181A] border-gray-50 dark:border-[#16181A]" :class="[u.estaActivo ? '!bg-green-500' : '']"></span>
            </div>
            <div class="min-w-0 flex-1">
              <p class="text-sm font-semibold truncate text-[#424242] dark:text-white">{{ u.nombreUsuario }}</p>
              <p class="text-xs truncate text-gray-500 dark:text-[#82A1B1]">{{ u.estaActivo ? 'En línea' : tiempoRelativo(u.ultimaConexion) }}</p>
            </div>
          </div>
        </div>
      </div>
...
.leaflet-tooltip.custom-dark-tooltip {
  background-color: rgba(39, 42, 48, 0.95); /* Color tarjeta Moveo */
  border: 1px solid rgba(55, 75, 84, 0.8); /* Borde azul Moveo */
  color: #ffffff;
  backdrop-filter: blur(8px);
...
.leaflet-tooltip.custom-dark-tooltip::before {
  border-top-color: rgba(39, 42, 48, 0.95);
}
