...
<template>
  <BaseModuleView
    ...
    description="Control de flota, mantenimientos y estados de vehículos"
    themeColor="emerald"
    :headerIcon="Truck"
    :cargando="cargando"
    v-model:searchQuery="busquedaVehiculos"
  ...
  >
    ...
    <template #custom-kpis>
      ...
      <div v-else class="col-span-4 grid grid-cols-1 md:grid-cols-4 gap-4">
        <!-- Tarjeta de Flota Total (Más grande) -->
        <div class="col-span-1 md:col-span-2 p-6 rounded-2xl border bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#374B54] flex items-center justify-between shadow-sm group">
          <div>
            <p class="text-sm font-medium text-gray-500 dark:text-[#82A1B1]">Total de la Flota</p>
            <h4 class="text-4xl font-black text-[#092C4C] dark:text-white mt-1">{{ vehiculos.length }}</h4>
            <div class="flex items-center gap-2 mt-4">
              <span class="flex items-center gap-1 text-xs font-semibold text-emerald-600 dark:text-emerald-400">
                <CheckCircle2 class="w-3.5 h-3.5" /> {{ vehiculos.filter(v => v.estado === 'DISPONIBLE').length }} Disponibles
              </span>
              <span class="text-gray-300 dark:text-gray-700">|</span>
              <span class="flex items-center gap-1 text-xs font-semibold text-blue-600 dark:text-blue-400">
                <Navigation class="w-3.5 h-3.5" /> {{ vehiculos.filter(v => v.estado === 'EN_RUTA').length }} Activos
              </span>
            </div>
          </div>
          <div class="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 transition-transform group-hover:scale-110">
            <Truck class="w-10 h-10 text-emerald-600 dark:text-emerald-400" />
          </div>
        </div>

        <!-- Estados Rápidos -->
        <div class="p-5 rounded-2xl border bg-emerald-600 text-white border-transparent shadow-lg shadow-emerald-200 dark:shadow-none">
          <Wrench class="w-6 h-6 mb-3 opacity-80" />
          <p class="text-xs font-medium opacity-80 uppercase tracking-widest text-emerald-100">En Taller</p>
          <div class="flex items-end justify-between mt-1">
            <span class="text-3xl font-bold">{{ vehiculos.filter(v => v.estado === 'EN_MANTENIMIENTO').length }}</span>
            <span class="text-xs font-medium bg-white/20 px-2 py-0.5 rounded backdrop-blur-md">Revisión</span>
          </div>
        </div>

        <div class="p-5 rounded-2xl border bg-white dark:bg-[#272A30] border-gray-100 dark:border-[#374B54] shadow-sm">
          <AlertCircle class="w-6 h-6 mb-3 text-red-500" />
          <p class="text-xs font-medium text-gray-500 dark:text-[#82A1B1] uppercase tracking-widest">Fuera Serv.</p>
          <div class="flex items-end justify-between mt-1">
            <span class="text-3xl font-bold text-[#092C4C] dark:text-white">{{ vehiculos.filter(v => v.estado === 'FUERA_DE_SERVICIO').length }}</span>
            <span class="text-xs font-medium text-red-500 bg-red-50 dark:bg-red-900/10 px-2 py-0.5 rounded">Alerta</span>
          </div>
        </div>
      </div>
    </template>
    ...
    <template #filters>
      <select v-model="filtroEstadoVehiculo" class="py-2.5 px-3 text-sm border rounded-xl focus:outline-none focus:border-emerald-500 transition-colors shadow-sm bg-white dark:bg-[#272A30] border-gray-200 dark:border-[#374B54] text-[#424242] dark:text-white cursor-pointer">
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-50 dark:divide-[#374B54]">
          <tr v-if="vehiculosFiltrados.length === 0">
             <td :colspan="esAdmin ? 8 : 7" class="py-20 text-center">
